...
var buff_readyCount = new JitterObject("jit.gpu.buffer");
var buff_tmpMin = new JitterObject("jit.gpu.buffer");
var buff_tmpMax = new JitterObject("jit.gpu.buffer");
...
comp_init_nodes.bind("buff_readyCount", buff_readyCount.name);
...
var comp_build_aabb = new JitterObject("jit.gpu.compute"); //build the structure from leaves to root
comp_build_aabb.shader = "comp_build_aabb.comp";
comp_build_aabb.bind("buff_nodeChild", buff_nodeChild.name);
comp_build_aabb.bind("buff_nodeParent", buff_nodeParent.name);
comp_build_aabb.bind("buff_nodeAabbMin", buff_nodeAabbMin.name);
comp_build_aabb.bind("buff_nodeAabbMax", buff_nodeAabbMax.name);
comp_build_aabb.bind("buff_readyCount", buff_readyCount.name);
comp_build_aabb.bind("buff_tmpMin", buff_tmpMin.name);
comp_build_aabb.bind("buff_tmpMax", buff_tmpMax.name);
...
function init_particles(x){
	...
	buff_readyCount.bytecount	= numNodes * 4;

	// tmp compatti: 2 slot per ciascun nodo interno
	buff_tmpMin.bytecount		= 2 * numInternal * 16;
	buff_tmpMax.bytecount		= 2 * numInternal * 16;
...
function bang(){
	...
	comp_gen_rand_pos.param("scale", 0.01);
